...
//TSGK
    ...
    function drawItems($image, $bounds, $data_array, $max_index, $name, $dot, $make_grid, $write_timestamp, $write_legend, $color)
	{
		...
		global $first_entry;
		global $theme;
		global $map_label_color;
		global $map_sep_color;
        global $map_block_color;
        global $fw;
        global $fh;
		$first_entry++;
...
if ($write_legend > 0) {
    ...
    $y_top = $bounds['indent_y'][0] - $fh - 4;
    $y_bottom = $bounds['indent_y'][0] - 6;
    ...
    imagestring(
        ...
        $theme['font_size'],
        $legend_x,
        $bounds['indent_y'][0] - $fh - 4,
        $name,
        $color[2]
    );

    $legend_x += ($fw * strlen($name)) + 7;
}
				...
				if (($entry['map'] !== $last_map) || ($pos['x'] <= ($bounds['indent_x'][0]+1) )) {
					if ($last_map == "") {
						...
						while ($last_map_posx > $pos['x']) {
							if (($bk_color % 2) == 0) {
								imageline($image, $last_map_posx, $bounds['indent_y'][0]+1, $last_map_posx, $bounds['height']-$bounds['indent_y'][1]-1, $map_block_color);
							}
							$last_map_posx--;
						}
						$bk_color++;
						imageline($image, $pos['x']+1, $bounds['indent_y'][0]+1, $pos['x']+1, $bounds['height']-$bounds['indent_y'][1]-1, $map_sep_color);
						$last_map_posx = $pos['x']; 
					}
				}
				$pos['x'] -= 3;
...
// END TSGK MAP TEXT
		...
		if ($make_grid > 0) {
			...
			while ($step_diff < 15) {
				$step_width++;
				if (floor($max_pos_y[$max_index]) % $step_width == 0) {
					$steps = floor($max_pos_y[$max_index] / $step_width);
					if ($steps > 0)
            ...
            $temp_y = round(($bounds['height']-$bounds['indent_y'][0]-$bounds['indent_y'][1]) - ((($bounds['height']-$bounds['indent_y'][0]-$bounds['indent_y'][1]) / $max_pos_y[$max_index]) * ($i*$step_width))) + $bounds['indent_y'][0];
                ...
                // compute label text
                if ($max_pos_y[$max_index] > 1000000) {
                    $str = sprintf("%.1fm", ($i*$step_width) / 1000000);
                } elseif ($max_pos_y[$max_index] > 10000) {
                    $str = sprintf("%.0fk", ($i*$step_width) / 1000);
                } else {
                    $str = sprintf("%.0f", $i*$step_width);
                }
                // compute label height
                static $last_y_label = null;
                ...
                if ($last_y_label === null || abs($temp_y - $last_y_label) >= ($fh + 4)) {
                    ...
                    // label
                    $str_pos = $bounds['indent_x'][0] - ($fw * strlen($str)) - 2;
            ...
            if (!isset($last_y_label) || $last_y_label === null || abs($temp_y - $last_y_label) >= ($fh + 4)) {
                if ($max_pos_y[$max_index] > 100000) {
                    $str     = sprintf("%.01fm", $max_pos_y[$max_index] / 1000000);
                } elseif ($max_pos_y[$max_index] > 10000) {
		    	...
		    	$str_pos = $bounds['indent_x'][0] - ($fw * strlen($str)) - 2;
		    	imagestring($image, $theme['font_size'], $str_pos, $bounds['indent_y'][0]- round($fh / 2), $str, $color[2]);
            }
		}
...
static $last_timestamp_x = null;
$timestamp_spacing = $fw * 6;   // enough room for "HH:MM"
		...
		foreach ($data_array as $key => $entry) {
			...
			if (($first_entry == 2) && ($server_load_type == 1)) {
				if ($entry['map'] !== $last_map) {
					if ($last_map == "") {
						$last_map      = $entry['map'];
						if ($entry['map'] != "" && ($last_map_posx - $pos['x']) >= ($fh+2) ) { 
							$str_height = $bounds['indent_y'][0] + ($fw * strlen($entry['map'])) + 2;
							imagestringup($image, $theme['font_size'], $pos['x']-$fh, $str_height, $entry['map'], $map_label_color);
						}
						$last_map_posx = $pos['x'];
					} else {
						$last_map = $entry['map'];
						$str_height = $bounds['indent_y'][0] + ($fw * strlen($entry['map'])) + 2;
                        if (($last_map_posx - $pos['x']) >= ($fh+2))
						imagestringup($image, $theme['font_size'], $pos['x']-$fh, $str_height, $entry['map'], $map_label_color);
						$last_map_posx = $pos['x'];
					}
				}
			...
			if ($key == 0) {
                ...
                if ($display_value > 1000000) {
					$str     = sprintf("%.1fm", $display_value / 1000000);
                    $str_pos = $pos['x'] + 4;
            } else if ($display_value > 10000) {
					$str     = sprintf("%.1fk", $display_value / 1000);
                    $str_pos = $pos['x'] + 4;
				} else {
                    if ($name === "max_players") {
                    
                       $max_val = $display_value;
                       $graph_h = $bounds['height'] - $bounds['indent_y'][0] - $bounds['indent_y'][1];

                       $pos['y'] = $bounds['indent_y'][0] +
                                   round($graph_h - ($graph_h / $max_pos_y[$max_index]) * $max_val);

                       imageline(
                           $image,
                           $bounds['indent_x'][0],
                           $pos['y'],
                           $bounds['width'] - $bounds['indent_x'][1],
                           $pos['y'],
                           $color[0]
                       );
                    
                       // label
                       $str = sprintf("%.0f", $max_val);
                       $str_pos = $bounds['indent_x'][0] - ($fw * strlen($str)) - 2;
                    } else  {
                ...
                if (empty($temp_y) || $pos['y'] !== $temp_y)
				    imagestring($image, $theme['font_size'], $str_pos, $pos['y'] - round($fh / 2), $str, $color[2]);
			}
        ...
        if ($last_timestamp_x !== null) { 
           ...
           //);
            ...
            imagestring(
                ...
                $theme['font_size'],
                $pos['x'] - ($fw * 2),
                $bounds['height'] - $bounds['indent_y'][1] + 3,
                $str,
                $color[2]
            );
        }

        $last_timestamp_x = $pos['x'];
    }
}
					break;
				...
				case 2:
					if (($write_timestamp > 0) && ($last_day > $this_day)) {
						...
						if ($first_day > 0) { 
							...
							imagestring($image, $theme['font_size'], $pos['x']-15, $bounds['height']-$bounds['indent_y'][1]+3, $str, $color[2]);
						}
						$first_day++;
					}
                    break;
