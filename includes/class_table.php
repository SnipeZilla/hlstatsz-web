...
<?php
		...
		{
			$totalwidth += 5;
			echo "<th style=\"width:5%;text-align:right;\" class=\"fSmall\">Rank</th>\n";
		}

		foreach ($this->columns as $col)
		{
			$totalwidth += $col->width;
			echo "<th style=\"width:$col->width%;text-align:$col->align;\" class=\"fSmall\">";
			if ($col->sort != 'no')
			{
			...
			foreach ($this->columns as $col)
			{
				...
				elseif ($col->flag)
				{
					...
					if ($g_options['countrydata'] == 1) { 
						...
						$cellbody .= '<span class="hlstats-flag"><img src="' . getFlag($rowdata['flag'])."\" class=\"tableicon\" alt=\"$alt_text\" title=\"$alt_text\" /></span>";
					}
					else
					{
						$col->flag = 'player';
				...
				switch ($col->type) {
					...
					default:
						//if ($this->showranking && $rank == 1 && $i == 1)
							$cellbody .= '<span class="hlstats-name">';
...
echo Pagination($numitems, $this->page, $this->numperpage, $this->var_page, $this->ajax);
		//}
	}
	...
	function _echoPageNumber ($number, $label, $prefix='', $postfix='')
	{
		...
		if ($this->ajax)
			echo "\" onclick=\"Tabs.refreshTab({'" . $this->var_page . "': " . $number . ",forceReload: true}); return false;";
