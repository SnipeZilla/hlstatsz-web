...
Awards (Player Actions) admin — direct SQL, no EditList dependency.
*/
...
$cr = $db->query(
	"SELECT code, description FROM hlstats_Actions
	 WHERE game='" . $db->escape($gamecode) . "' AND for_PlayerActions='1'
...
$formAction = htmlspecialchars(
	$g_options['scripturl'] . '?mode=admin&task=awards_plyractions&game=' . urlencode($gamecode) . '&page=' . $currentPage
);
?>
